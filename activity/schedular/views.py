...
class ProjectAnalyticsViewSet(viewsets.GenericViewSet):
    ...
    def projects_analytics(self, request):
        """
        Get projects with planned/achieved hours and multi-level filtering.
        
        Query Parameters:
        - project_id: Filter by specific project (Scenario 2, 4)
        - employee_id: Filter by specific employee (Scenario 3, 5)
        - user_id: Alias for employee_id (for compatibility with dashboard providers)
        - start_date: ISO format date (YYYY-MM-DD)
        - end_date: ISO format date (YYYY-MM-DD)
        
        Returns:
        - List of projects with planned/achieved hours aggregated from tasks
        - Root-level aggregates (total_planned_hours, total_achieved_hours)
        - all_projects list for the dashboard dropdown
        """
        project_id = request.query_params.get('project_id')
        ...
        # We want to see stats for Active and Completed projects, and On Hold too.
        # This fixes the issue where completed projects disappeared from stats.
        valid_statuses = ['ACTIVE', 'COMPLETED', 'ON HOLD']
        projects_qs = Projects.objects.filter(status__in=valid_statuses)
        
        # Apply ownership/role filters
        user_role = getattr(self.request.user, 'role', 'EMPLOYEE')
        
        if project_id and not employee_id:
            projects_qs = projects_qs.filter(id=project_id)
        elif employee_id and not project_id:
            # Multi-role project membership
            projects_qs = projects_qs.filter(
                models.Q(tasks__assignees__user_id=employee_id) | 
                models.Q(project_lead_id=employee_id) |
                models.Q(handled_by_id=employee_id) |
                models.Q(created_by_id=employee_id)
            ).distinct()
        elif project_id and employee_id:
            projects_qs = projects_qs.filter(
                id=project_id,
                tasks__assignees__user_id=employee_id
            ).distinct()
        elif user_role != 'ADMIN':
            # Default for non-admin: show only THEIR projects
            me = self.request.user
            projects_qs = projects_qs.filter(
                models.Q(tasks__assignees__user=me) | 
                models.Q(project_lead=me) |
                models.Q(handled_by=me) |
                models.Q(created_by=me)
            ).distinct()
        ...
        def parse_dt(dt_str):
            ...
            5. Select Employee 
            """
            # Provide a base queryset and serializer so DRF generic machinery
            # (including OrderingFilter) is satisfied, even though our actions
        ...
        for project in projects_qs:
            # We must filter tasks by employee if requested
            tasks_qs = project.tasks.all()
            if employee_id:
                tasks_qs = tasks_qs.filter(assignees__user_id=employee_id).distinct()
            
            # Task counts for this project slice
            project_total_tasks = tasks_qs.count()
            project_done_tasks = tasks_qs.filter(progress=100).count()
            
            total_tasks_count += project_total_tasks
            completed_tasks_count += project_done_tasks
            
            # Optimized: Sum up planned hours without per-project DB hits
            p_planned = sum(float(t.planned_hours or 0.0) for t in tasks_qs)
            
            # Sum up achieved hours from ActivityLog in one project-specific query
            logs_filter = {'today_plan__catalog_item__task__project': project, 'status__in': ['COMPLETED', 'PENDING']}
            if employee_id: logs_filter['user_id'] = employee_id
            
            logs_qs = ActivityLog.objects.filter(**logs_filter)
            if s_date: logs_qs = logs_qs.filter(today_plan__plan_date__gte=s_date)
            if e_date: logs_qs = logs_qs.filter(today_plan__plan_date__lte=e_date)
            
            p_achieved = float(logs_qs.aggregate(total=Sum('hours_worked'))['total'] or 0.0)
            total_planned += p_planned
            total_achieved += p_achieved

            # We need task-level data for the single-project donut chart
            tasks_data = []
            if project_id: # Detailed task data only when an individual project is selected
                for t in tasks_qs:
                    t_logs = logs_qs.filter(today_plan__catalog_item__task=t)
                    t_achieved = float(t_logs.aggregate(total=Sum('hours_worked'))['total'] or 0.0)
                    tasks_data.append({
                        'id': t.id,
                        'name': t.title,
                        'planned_hours': float(t.planned_hours or 0.0),
                        'achieved_hours': t_achieved,
                    })
            ...
            projects_list.append({
                ...
                'name': project.name,
                'status': project.status,
                'project_budget': project.planned_hours, 
                'planned_hours': p_planned, 
                'achieved_hours': p_achieved,
                'progress': min(100, int((p_achieved / p_planned * 100) if p_planned > 0 else 0)),
                'task_count': project_total_tasks,
                'completed_tasks_count': project_done_tasks,
                'tasks': tasks_data
            })
            
        # Dropdown logic for project selection
        all_projects_qs = Projects.objects.filter(status__in=valid_statuses)
        if employee_id:
            all_projects_qs = all_projects_qs.filter(
                Q(tasks__assignees__user_id=employee_id) | 
                Q(project_lead_id=employee_id) |
                Q(handled_by_id=employee_id) |
                Q(created_by_id=employee_id)
            ).distinct()
        elif user_role != 'ADMIN':
            me = self.request.user
            all_projects_qs = all_projects_qs.filter(
                Q(tasks__assignees__user=me) | 
                Q(project_lead=me) |
                Q(handled_by=me) |
                Q(created_by=me)
            ).distinct()
            
        all_projects_list = [{'id': p.id, 'name': p.name} for p in all_projects_qs]
        ...
        return Response({
            ...
            'overall_completion_percentage': min(100, round((total_achieved / total_planned * 100) if total_planned > 0 else 0)),
            'filters': {
